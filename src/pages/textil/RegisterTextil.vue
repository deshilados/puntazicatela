            ...
            <div class="card-body p-4">
              ...
              <form @submit.prevent="submit">
                <div class="mb-3">
                  ...
                  <input v-model="nombre" type="text" class="form-control" id="nombre" required autocomplete="name" />
                </div>
                <div class="mb-3">
...
async function submit() {
  error.value = '';
  const { data: existing } = await supabase.from('users').select('id').eq('email', email.value.trim().toLowerCase()).single();
  if (existing) {
